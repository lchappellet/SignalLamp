...
double Filter_ultrasonic::average_ping_filter() {
    ...
    avg_distance_total = total_distance_value / count;
...
#define TRIGGER_PIN \
  10                // Arduino pin tied to trigger pin on the ultrasonic sensor.
...
#define light_yellow 6
  ...
  */
  ...
  rn on green light when there is no boxes past a certain point.*
      ...
      have just gotten less.*Turn on the Red light when the boxes have reached a
                                 certain hieght.*
              Turn on a small noise when the lights change to red.***** /
